...
export default async function Dashboard() {
  ...
  return (
    <>
      ...
      <section className="card"><div className="dashboardSectionHeader"><div><h2>Continue Working</h2><p className="muted">Recent Projects and their current workbook status.</p></div><Link className="secondary" href="/projects">View all Projects</Link></div>{projects.length ? <div className="projectGrid">{projects.map((project) => { const workbook = project.sourceWorkbookImports[0]; const workbookName = workbook ? friendlyWorkbookName(workbook.originalFileName) : "Workbook not uploaded"; return <Link aria-label={`Open Project ${project.name}`} className="projectCard projectCardLink" href={`/projects/${project.id}`} key={project.id}><strong className="projectCardTitle" title={project.name}>{project.name}</strong><span className="projectCardWorkbook" title={workbookName}>{workbookName}</span><small className="projectCardMeta">{workbook?.validationStatus === "VALIDATED" ? "Ready" : workbook ? "Needs review" : "Upload a workbook to start"} · Updated {project.updatedAt.toLocaleDateString("en-GB")}</small></Link>; })}</div> : <div className="emptyState"><p className="muted">No Projects yet.</p>{canCreate ? <Link className="primary" href="/projects/new">Create Project</Link> : null}</div>}</section>
