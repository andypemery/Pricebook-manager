...
import { configuredWorksheetName, validateWorksheetNames } from "@/lib/data-mapper/output-profiles/configuration";
...
export function OutputGenerationPanel({ sourceWorkbookImportId, draft, worksheets = [], filenameDate, canEdit }: { sourceWorkbookImportId: string; draft: OutputProfileDraft; worksheets?: SourceWorkbookWorksheet[]; filenameDate: string; canEdit: boolean }) {
  ...
  const invalidSelection = selectedWorksheetIds.length === 0
    || (worksheets.length > 0 && selectedWorksheets.length !== selectedWorksheetIds.length)
    || selectedWorksheets.some((worksheet) => !resolveWorksheetCompatibility(draft, worksheet.headers).compatible);
  const invalidWorksheetNames = draft.worksheetMode !== "COMBINE"
    && validateWorksheetNames(selectedWorksheets.map((worksheet) => configuredWorksheetName(draft, worksheet.name))).length > 0;
  ...
  async function generate() {
    ...
    try {
      ...
      setMessage(`${worksheetCount.toLocaleString("en-GB")} ${worksheetCount === 1 ? "worksheet" : "worksheets"} processed · ${rowCount.toLocaleString("en-GB")} rows generated and downloaded.${zeroRows.length > 0 ? ` ${zeroRows.length} ${zeroRows.length === 1 ? "worksheet produced" : "worksheets produced"} 0 matching rows: ${zeroRows.join(", ")}.` : ""}`);
  ...
  return <section className="card outputGenerationPanel" aria-labelledby="generate-output-title">
    <div className="sectionHeader"><div><p className="sheetLabel">Final step</p><h2 id="generate-output-title">Generate output file</h2><p className="muted">The private source workbook is fetched and parsed once on the server. Ignoring an error does not correct its source value.</p></div></div>
    ...
    {state ? <>
      <div className="summaryGrid"><div><strong>{blockingIssues.length}</strong><span>Blocking errors</span></div><div><strong>{ignoredBlockingCount}</strong><span>Ignored</span></div><div><strong>{unresolvedBlockingCount}</strong><span>Unresolved</span></div><div><strong>{warningCount}</strong><span>Warnings</span></div></div>
      {unresolvedBlockingCount > 0 ? <p className="validationErrorGuidance">{unresolvedBlockingCount} unresolved blocking {unresolvedBlockingCount === 1 ? "error must" : "errors must"} be resolved or ignored before generation.</p> : blockingIssues.length > 0 ? <p className="validationWarningGuidance">All {blockingIssues.length} blocking {blockingIssues.length === 1 ? "error is" : "errors are"} currently ignored. Generation is permitted.</p> : <p className="success">No blocking validation errors prevent generation.</p>}
      {relevantIssues.length > 0 ? <div className="actions validationReviewActions"><button className="secondary" type="button" disabled={!canEdit || pending || unresolvedBlockingCount === 0} onClick={() => change(blockingIssues.filter((issue) => !issue.ignored).map((issue) => issue.fingerprint), "IGNORE", `Ignore all ${unresolvedBlockingCount} blocking errors?\n\nThis does not correct the source workbook.`)}>Ignore all blocking errors</button><button className="secondary" type="button" disabled={!canEdit || pending || ignoredBlockingCount === 0} onClick={() => change(blockingIssues.filter((issue) => issue.ignored).map((issue) => issue.fingerprint), "RESTORE", `Restore all ${ignoredBlockingCount} ignored errors?`)}>Restore all ignored errors</button><button className="secondary" type="button" onClick={() => setReviewOpen(true)}>Review validation issues</button></div> : null}
      <div className="actions"><button className="primary" type="button" disabled={!canEdit || pending || unresolvedBlockingCount > 0 || draft.columns.length === 0 || invalidSelection || invalidWorksheetNames || (draft.worksheetMode === "SEPARATE_WORKSHEETS" && draft.outputFormat !== "XLSX")} onClick={generate}><Download aria-hidden="true" size={18} /> {pending ? "Generating" : "Generate output file"}</button></div>
